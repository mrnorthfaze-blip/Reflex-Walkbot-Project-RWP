...
public:
    ...
    static constexpr FNV1A_t OFFSET_BASIS = 0xcbf29ce484222325ull;
    static constexpr FNV1A_t PRIME = 0x100000001b3ull;

    static FNV1A_t Hash(const char* szString, FNV1A_t uHash = OFFSET_BASIS)
    {
        while (*szString)
        {
            uHash ^= *szString++;
...
typedef long(*tNtReadVirtualMemory)(HANDLE ProcessHandle, PVOID BaseAddress, PVOID Buffer, ULONG NumberOfBytesToRead, PULONG NumberOfBytesRead);
typedef long(*tNtWriteVirtualMemory)(HANDLE ProcessHandle, PVOID BaseAddress, PVOID Buffer, ULONG NumberOfBytesToWrite, PULONG NumberOfBytesWritten);
...
public:
    ...
    ~CMemory()
    {
        if (m_bOwnsProcessHandle && m_pProcessHandle)
            CloseHandle(m_pProcessHandle);
    }

    bool IsAttached() const
    {
        return m_pProcessHandle != nullptr && m_pProcessID != 0;
    }

    void AttachToProcess(HANDLE hProcess, DWORD dwProcessId, bool bTakeOwnership = false)
    {
        if (m_bOwnsProcessHandle && m_pProcessHandle && m_pProcessHandle != hProcess)
            CloseHandle(m_pProcessHandle);

        m_pProcessHandle = hProcess;
        m_pProcessID = dwProcessId;
        m_bOwnsProcessHandle = bTakeOwnership;
        m_mapModules.clear();
    }
    ...
    void Detach()
    {
        ...
        m_pProcessHandle = nullptr;
        m_pProcessID = 0;
        m_bOwnsProcessHandle = false;
        m_mapModules.clear();
    }
    ...
    void Initialize(const char* szProcessName)
    {
        ...
        while (!m_pProcessID)
        {
            ...
            {
                // Convert WCHAR szExeFile to char array for comparison
                char szExeName[MAX_PATH] = {};
                for (int i = 0; entry.szExeFile[i] && i < MAX_PATH - 1; ++i)
                    szExeName[i] = static_cast<char>(entry.szExeFile[i]);

                if (strcmp(szExeName, szProcessName) == 0)
                {
                    const DWORD dwProcessId = entry.th32ProcessID;
                    HANDLE hProcess = OpenProcess(PROCESS_VM_READ | PROCESS_VM_WRITE, FALSE, dwProcessId);
                    if (hProcess)
                    {
                        AttachToProcess(hProcess, dwProcessId, true);
                        break;
                    }
                }
            }
    ...
    ModuleInformation_t GetModule(const char* szModuleName)
    {
        ...
        if (m_mapModules.find(uHash) == m_mapModules.end())
        {
            ...
            {
                // Convert WCHAR szModule to char array for comparison
                char szModuleName_conv[MAX_PATH] = {};
                for (int i = 0; entry.szModule[i] && i < MAX_PATH - 1; ++i)
                    szModuleName_conv[i] = static_cast<char>(entry.szModule[i]);

                if (strcmp(szModuleName, szModuleName_conv) == 0)
                {
                    if (hSnapShot)
                        CloseHandle(hSnapShot);

                    // Convert WCHAR szExePath to char array
                    char szExePath[MAX_PATH] = {};
                    for (int i = 0; entry.szExePath[i] && i < MAX_PATH - 1; ++i)
                        szExePath[i] = static_cast<char>(entry.szExePath[i]);

                    m_mapModules[uHash] = { reinterpret_cast<std::uintptr_t>(entry.modBaseAddr), std::string(szExePath), static_cast<std::uintptr_t>(entry.modBaseSize) };
    ...
    std::uintptr_t PatterScan(const char* szModuleName, const char* szSignature, uint16_t uFlags = NO_FLAGS, std::uint32_t uOption1 = 0U, std::uint32_t uOption2 = 0U)
    {
        ...
        if (!moduleInformation.m_uBaseAddress || !moduleInformation.m_uModuleSize)
            return 0U;
        ...
        {
            printf("Failed to read module bytes\n");
            delete[] pModuleBytes;
            return 0U;
        }
        ...
        if (uAddress == 0U)
        {
            printf("Failed to find signature in %s\n", szModuleName);
            delete[] pModuleBytes;
            return 0U;
        }
...
private:
    ...
    bool m_bOwnsProcessHandle = false;
